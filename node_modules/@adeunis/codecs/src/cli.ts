...
class AdeunisCodecsCli {
  ...
  public run() {
    ...
    switch (this.args[0]) {
      ...
      case 'decode':
        ...
        const frames = this.options._.slice(1).map(o => (typeof o !== 'string' ? (o as any).toString() : o));
  ...
  public helpDecode() {
    ...
    Object.values(codec.DecoderProducts).forEach(function (val: string, index: number) {
  ...
  public helpEncode(deviceType: string, frameCode: any) {
    if (deviceType && frameCode !== 0) {
      ...
      console.log(
        `${chalk.bold('Usage:')} codec ${chalk.blue('encode')} <device_type> <frame_code>` +
...
======================================== ${section.title} ========================================`);
   ...
   * @param deviceType device type
   */
  private printDecoded(frames: string[], devId: string, network: any, deviceType: string, csvOutput: boolean, jsonOutput: boolean) {
    ...
    for (const payloadString of frames) {
      ...
      if (csvOutput) {
        ...
        console.log(json2csv(content as object[], options));
        continue;
      }
