...
describe('Motion0x5cParser', () => {
  ...
  it('should parse 5c80001e320020', () => {
    const content = parser.parseFrame(Buffer.from('5c80001e320020', 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x5c Motion data');
    expect(content['presenceDetectedWhenSending']).to.equal(false);
    expect(content['presence'].values[0]).to.equal(30);
    expect(content['luminosity'].values[0]).to.equal(50);
    expect(content['presence'].values[1]).to.equal(0);
    expect(content['luminosity'].values[1]).to.equal(32);
  ...
  it('should parse 5c800000320020', () => {
    const content = parser.parseFrame(Buffer.from('5c800000320020', 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x5c Motion data');
    expect(content['presenceDetectedWhenSending']).to.equal(false);
    expect(content['presence'].values[0]).to.equal(0);
    expect(content['luminosity'].values[0]).to.equal(50);
    expect(content['presence'].values[1]).to.equal(0);
    expect(content['luminosity'].values[1]).to.equal(32);
  });
