...
describe('Drycontacts0x40Parser', () => {
  let parser: codec.Drycontacts20x40Parser;

  beforeEach(() => {
    parser = new codec.Drycontacts20x40Parser();
  });
  ...
  it('should parse 4040000100000005000201', () => {
    ...
    expect(content.type).to.equal('0x40 Dry Contacts 2 data');
  ...
  it('should parse 404400010000000500020114ABA3E9', () => {
    const payloadString = '404400010000000500020114ABA3E9';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x40 Dry Contacts 2 data');
