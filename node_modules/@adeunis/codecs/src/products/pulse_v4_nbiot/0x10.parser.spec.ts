...
describe('PulseV4NbIot0x10Parser', () => {
  let parser: codec.PulseV4NbIot0x10Parser;

  beforeEach(() => {
    parser = new codec.PulseV4NbIot0x10Parser();
  });

  it('should parse 0x10 frame', () => {
    const payloadString = '1122334455667788030000000010000100021B070822003c0001000200030004000500060203020301';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x10 Pulse 4 NB-IoT configuration');
