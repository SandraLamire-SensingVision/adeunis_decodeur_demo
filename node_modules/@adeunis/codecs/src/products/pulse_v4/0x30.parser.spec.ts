...
describe('PulseV40x30Parser', () => {
  let parser: codec.PulseV40x30Parser;

  beforeEach(() => {
    parser = new codec.PulseV40x30Parser();
  });
  ...
  it('should parse 30e0150400030002000100', () => {
    ...
    expect(content.type).to.equal('0x30 Pulse 4 keep alive');
  ...
  it('should parse 303415040003000200010014ABA3E9', () => {
    const payloadString = '303415040003000200010014ABA3E9';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x30 Pulse 4 keep alive');
    expect(content['channels'][0].name).to.equal('channel A');
    expect(content['channels'][0].flow.alarm).to.equal(true);
