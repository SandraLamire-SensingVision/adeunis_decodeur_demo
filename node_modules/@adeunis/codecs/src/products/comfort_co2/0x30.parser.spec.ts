...
describe('ComfortCo20x30Parser', () => {
  let parser: codec.ComfortCo20x30Parser;

  beforeEach(() => {
    parser = new codec.ComfortCo20x30Parser();
  });
  it('should parse frame and return correct config 1', () => {
    const payloadString = '3001039001F403E8460064';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x30 Comfort CO2 Daily frame');
  ...
  it('should parse frame and return correct config 2', () => {
    const payloadString = '3001039001F403E8460064000A0A00c5025832';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x30 Comfort CO2 Daily frame');
    expect(content['icone']).to.equal(3);
    expect(content['qaiRedDuration'].value).to.equal(1440);
  ...
  it('should parse frame and return correct config 3', () => {
    const payloadString = '3084039001F403E8460064000A0A00c502583214ABA3E9';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x30 Comfort CO2 Daily frame');
