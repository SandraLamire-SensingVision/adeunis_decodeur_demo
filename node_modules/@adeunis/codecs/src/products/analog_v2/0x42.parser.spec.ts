...
describe('Analog20x42Parser', () => {
  let parser: codec.Analog20x42Parser;
  ...
  it('should parse 42400100000000000000', () => {
    ...
    expect(content.type).to.equal('0x42 Analog V2 data');
  ...
  it('should parse 424802124F80017A1200', () => {
    const payloadString = '424002124F80017A1200';
    const content = parser.parseFrame(Buffer.from(payloadString, 'hex'), Buffer.from(''), 'unknown');
    expect(content.type).to.equal('0x42 Analog V2 data');
    expect(content['channels'][0].name).to.equal('channel A');
    expect(content['channels'][0].unit).to.equal('mA');
    expect(content['channels'][0].value).to.equal(12);
    expect(content['channels'][1].name).to.equal('channel B');
    expect(content['channels'][1].unit).to.equal('V');
    expect(content['channels'][1].value).to.equal(8);
  });
});
