...
describe('Repeater0x03Parser', () => {
  ...
  it('should parse data 03A30203 ', () => {
    ...
    expect(content['lowBattery']).to.equal(true);
    expect(content['downlinkCode']).to.equal('0x02 Delete an ID from White List');
    expect(content['downlinkErrorCode']).to.equal('0x03 White List is empty, repeater switch into OPERATION with “auto-record” mode');
  });
  ...
  it('should parse data 03A30507 ', () => {
    ...
    expect(content['frameCounter']).to.equal(10);
    expect(content['lowBattery']).to.equal(true);
    expect(content['downlinkCode']).to.equal('0x05 Freeze the list of devices repeated in auto-record mode into the White List');
    expect(content['downlinkErrorCode']).to.equal('0x07 No ID repeated, repeater stay into OPERATION with “auto-record” mode');
  });
});
